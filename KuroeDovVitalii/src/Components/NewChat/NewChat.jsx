...
export default function NewChat(props) {
    ...
    const handleChange = (event) => {
        const searchUser = users.find((item) => item.id === event.target.value)
        setSelectedUser({
            id: searchUser.id,
            ...
            >
                ...
                >
                    <div className={classes.profileTitleContainer}>
                        Новый чат
                        {selectedUser.name ? `с ${selectedUser.name}` : null}
                    </div>
                </DialogTitle>
                ...
                <DialogActions>
                    <Button
                        ...
                        color="primary"
                        variant={
                            selectedUser.name.length > 0 ? "text" : "contained"
                        }
                    >
                        Отмена
                    </Button>
                    <Button
                        autoFocus
                        variant={
                            selectedUser.name.length > 0 ? "contained" : "text"
                        }
